...
void ScriptModule::configureA()
{
    if( !m_configureA ) return;

    prepare( m_configureA );
    m_context->SetArgDWord( 0, m_cfgRegA->read() );
    execute();
}

void ScriptModule::configureB()
{
    if( !m_configureB ) return;

    prepare( m_configureB );
    m_context->SetArgDWord( 0, m_cfgRegB->read() );
    execute();
}

void ScriptModule::configureC()
{
    if( !m_configureC ) return;

    prepare( m_configureC );
    m_context->SetArgDWord( 0, m_cfgRegC->read() );
    execute();
}
