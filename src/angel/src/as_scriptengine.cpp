...
int asCScriptEngine::SetEngineProperty(asEEngineProp property, asPWORD value)
{
    ...
    case asEP_SCRIPT_SCANNER:
        if( value <= 1 ) ep.scanner = (int)value;
        else             return asINVALID_ARG;
        break;
    ...
    case asEP_STRING_ENCODING:
        if( value <= 1 ) ep.stringEncoding = (int)value;
        else             return asINVALID_ARG;
        break;

    case asEP_PROPERTY_ACCESSOR_MODE:
        if( value <= 3 ) ep.propertyAccessorMode = (int)value;
        else             return asINVALID_ARG;
        break;
    ...
    case asEP_COMPILER_WARNINGS:
        if( value <= 2 ) ep.compilerWarnings = (int)value;
        else             return asINVALID_ARG;
        break;
    ...
    case asEP_ALTER_SYNTAX_NAMED_ARGS:
        if( value <= 2 ) ep.alterSyntaxNamedArgs = (int)value;
        else             return asINVALID_ARG;
        break;
    ...
    case asEP_HEREDOC_TRIM_MODE:
        if (value <= 2) ep.heredocTrimMode = (int)value;
        else            return asINVALID_ARG;
        break;

    case asEP_MAX_NESTED_CALLS:
        if (value > 0xFFFFFFFF) ep.maxNestedCalls = 0xFFFFFFFF;
        else                    ep.maxNestedCalls = (asUINT)value;
        break;

    case asEP_GENERIC_CALL_MODE:
        if (value > 1) ep.genericCallMode = 1;
        else           ep.genericCallMode = (asUINT)value;
        break;
...
asPWORD asCScriptEngine::GetEngineProperty(asEEngineProp property) const
{
    switch( property )
    {
    case asEP_ALLOW_UNSAFE_REFERENCES:            return ep.allowUnsafeReferences;
    case asEP_COPY_SCRIPT_SECTIONS:               return ep.copyScriptSections;
    case asEP_MAX_STACK_SIZE:                     return ep.maximumContextStackSize * 4;
    case asEP_INIT_STACK_SIZE:                    return ep.initContextStackSize * 4;
    case asEP_USE_CHARACTER_LITERALS:             return ep.useCharacterLiterals;
    case asEP_ALLOW_MULTILINE_STRINGS:            return ep.allowMultilineStrings;
    case asEP_ALLOW_IMPLICIT_HANDLE_TYPES:        return ep.allowImplicitHandleTypes;
    case asEP_BUILD_WITHOUT_LINE_CUES:            return ep.buildWithoutLineCues;
    case asEP_INIT_GLOBAL_VARS_AFTER_BUILD:       return ep.initGlobalVarsAfterBuild;
    case asEP_REQUIRE_ENUM_SCOPE:                 return ep.requireEnumScope;
    case asEP_SCRIPT_SCANNER:                     return ep.scanner;
    case asEP_INCLUDE_JIT_INSTRUCTIONS:           return ep.includeJitInstructions;
    case asEP_STRING_ENCODING:                    return ep.stringEncoding;
    case asEP_PROPERTY_ACCESSOR_MODE:             return ep.propertyAccessorMode;
    case asEP_EXPAND_DEF_ARRAY_TO_TMPL:           return ep.expandDefaultArrayToTemplate;
    case asEP_AUTO_GARBAGE_COLLECT:               return ep.autoGarbageCollect;
    case asEP_ALWAYS_IMPL_DEFAULT_CONSTRUCT:      return ep.alwaysImplDefaultConstruct;
    case asEP_COMPILER_WARNINGS:                  return ep.compilerWarnings;
    case asEP_DISALLOW_VALUE_ASSIGN_FOR_REF_TYPE: return ep.disallowValueAssignForRefType;
    case asEP_ALTER_SYNTAX_NAMED_ARGS:            return ep.alterSyntaxNamedArgs;
    case asEP_DISALLOW_EMPTY_LIST_ELEMENTS:       return ep.disallowEmptyListElements;
    case asEP_PRIVATE_PROP_AS_PROTECTED:          return ep.privatePropAsProtected;
    case asEP_ALLOW_UNICODE_IDENTIFIERS:          return ep.allowUnicodeIdentifiers;
    case asEP_HEREDOC_TRIM_MODE:                  return ep.heredocTrimMode;
    case asEP_MAX_NESTED_CALLS:                   return ep.maxNestedCalls;
    case asEP_GENERIC_CALL_MODE:                  return ep.genericCallMode;
    case asEP_INIT_CALL_STACK_SIZE:               return ep.initCallStackSize;
    case asEP_MAX_CALL_STACK_SIZE:                return ep.maxCallStackSize;
    default:                                      return 0;
    }
    UNREACHABLE_RETURN;
}
...
void asCScriptEngine::DeleteDiscardedModules()
{
    ...
    for( asUINT n = 0; n < globalProperties.GetLength(); n++ )
    {
        ...
        if( prop && prop->refCount.get() == 1 ) RemoveGlobalProperty(prop);
    }
}
...
asCScriptEngine::~asCScriptEngine()
{
    ...
    for( asUINT n = 0; n < generatedTemplateTypes.GetLength(); n++ )
    {
        ...
        if( templateType ) templateType->DestroyInternal();
    }
    ...
    for( asUINT n = 0; n < templateInstanceTypes.GetLength(); n++ )
    {
        ...
        if( templateInstanceTypes[n] ) templateType->ReleaseInternal();
    }
    ...
    for( ; funcIt; funcIt++ ) (*funcIt)->ReleaseInternal();
    ...
    // Free namespaces
    for( asUINT n = 0; n < nameSpaces.GetLength(); n++ ) asDELETE(nameSpaces[n], asSNameSpace);
...
asCModule *asCScriptEngine::FindNewOwnerForSharedType(asCTypeInfo *in_type, asCModule *in_mod)
{
    ...
    if( in_type->module != in_mod) return in_type->module;
...
void asCScriptEngine::ReturnContext(asIScriptContext *ctx)
{
    ...
    if( ctx ) ctx->Release(); // As fallback we just release the context
}
...
asSNameSpace *asCScriptEngine::AddNameSpace(const char *name)
{
    ...
    if( ns == 0 ) return 0; // Out of memory
...
int asCScriptEngine::WriteMessage(const char *section, int row, int col, asEMsgType type, const char *message)
{
    ...
    if( msgCallbackFunc.callConv < ICC_THISCALL )
        CallGlobalFunction(&msg, msgCallbackObj, &msgCallbackFunc, 0);
    else CallObjectMethod(msgCallbackObj, &msg, &msgCallbackFunc, 0);
...
{
    if( stringLength == 0 ) stringLength = strlen(string);

    size_t len;
    asETokenClass tc;
    tok.GetToken(string, stringLength, &len, &tc);

    if( tokenLength ) *tokenLength = (asUINT)len;
...
asCString asCScriptEngine::GetFunctionDeclaration(int funcId)
{
    ...
    if( func ) str = func->GetDeclarationStr();
...
int asCScriptEngine::CreateContext(asIScriptContext **context, bool isInternal)
{
    *context = asNEW(asCContext)(this, !isInternal);
    if( *context == 0 ) return asOUT_OF_MEMORY;
...
int asCScriptEngine::RegisterObjectProperty( const char *obj, const char *declaration, int byteOffset, int compositeOffset, bool isCompositeIndirect)
{
    ...
    if( r < 0 ) return ConfigError(r, "RegisterObjectProperty", obj, declaration);
    ...
    if( prop == 0 ) return ConfigError(asOUT_OF_MEMORY, "RegisterObjectProperty", obj, declaration);
...
int asCScriptEngine::RegisterObjectType( const char *name, int byteSize, asDWORD flags )
{
    ...
    else return ConfigError(asINVALID_ARG, "RegisterObjectType", name, 0);
    ...
    // Verify type name
    if( name == 0 ) return ConfigError(asINVALID_NAME, "RegisterObjectType", name, 0);
    ...
    if( flags & asOBJ_TEMPLATE )
    {
        ...
        if( r < 0 ) return ConfigError(r, "RegisterObjectType", name, 0);
        ...
        if( type == 0 ) return ConfigError(asOUT_OF_MEMORY, "RegisterObjectType", name, 0);
...
//                return ConfigError(asNOT_SUPPORTED, "RegisterObjectType", name, 0);
    ...
    if( flags & asOBJ_TEMPLATE ) return asSUCCESS;
...
int asCScriptEngine::RegisterObjectBehaviour(const char *datatype, asEBehaviours behaviour, const char *decl, const asSFuncPtr &funcPointer, asDWORD callConv, void *auxiliary, int compositeOffset, bool isCompositeIndirect)
{
    ...
    if( r < 0 ) return ConfigError(r, "RegisterObjectBehaviour", datatype, decl);
...
int asCScriptEngine::RegisterBehaviourToObjectType(asCObjectType *objectType, asEBehaviours behaviour, const char *decl, const asSFuncPtr &funcPointer, asDWORD callConv, void *auxiliary, int compositeOffset, bool isCompositeIndirect)
{
    ...
    if( r < 0 )
    {
        if( listPattern ) listPattern->Destroy(this);
    ...
    else if( behaviour == asBEHAVE_DESTRUCT )
    {
        ...
        if( beh->destruct ) return ConfigError(asALREADY_REGISTERED, "RegisterObjectBehaviour", objectType->name.AddressOf(), decl);
    ...
    else if( behaviour == asBEHAVE_LIST_CONSTRUCT )
    {
        ...
        if( r < 0 ) return ConfigError(r, "RegisterObjectBehaviour", objectType->name.AddressOf(), decl);
    }
    ...
    if( func.id < 0 ) return ConfigError(func.id, "RegisterObjectBehaviour", objectType->name.AddressOf(), decl);
...
int asCScriptEngine::AddBehaviourFunction(asCScriptFunction &func, asSSystemFunctionInterface &internal)
{
    ...
    if( newInterface == 0 ) return asOUT_OF_MEMORY;
    ...
    if( f->objectType ) f->objectType->AddRefInternal();
    f->id             = id;
...
int asCScriptEngine::RegisterGlobalProperty(const char *declaration, void *pointer)
{
    ...
    if( pointer == 0 ) return ConfigError(asINVALID_ARG, "RegisterGlobalProperty", declaration, 0);
    ...
    if( type.IsReference() ) return ConfigError(asINVALID_TYPE, "RegisterGlobalProperty", declaration, 0);
...
void asCScriptEngine::RemoveGlobalProperty(asCGlobalProperty *prop)
{
    ...
    if( index >= 0 )
    {
        ...
        asASSERT(node);
        if( node ) varAddressMap.Erase(node);
...
{
    if( obj == 0 ) return ConfigError(asINVALID_ARG, "RegisterObjectMethod", obj, declaration);
...
int asCScriptEngine::RegisterGlobalFunction(const char *declaration, const asSFuncPtr &funcPointer, asDWORD callConv, void *auxiliary)
{
    ...
    // Put the system function in the list of system functions
    asSSystemFunctionInterface *newInterface = asNEW(asSSystemFunctionInterface)(internal);
    if( newInterface == 0 ) return ConfigError(asOUT_OF_MEMORY, "RegisterGlobalFunction", declaration, 0);
    ...
    if( func->IsProperty() && (r = bld.ValidateVirtualProperty(func)) < 0 )
    {
        func->funcType = asFUNC_DUMMY; // Set as dummy function before deleting
        asDELETE(func,asCScriptFunction);
        if( r == -5 ) return ConfigError(asNAME_TAKEN, "RegisterGlobalFunction", declaration, 0);
        else          return ConfigError(asINVALID_DECLARATION, "RegisterGlobalFunction", declaration, 0);
    }
...
asIScriptFunction *asCScriptEngine::GetGlobalFunctionByIndex(asUINT index) const
{
    ...
    if( index >= registeredGlobalFuncs.GetSize() ) return 0;
...
void asCScriptEngine::PrepareEngine()
{
    ...
    for( n = 0; n < scriptFunctions.GetLength(); n++ )
    {
        ...
        if( scriptFunctions[n] && scriptFunctions[n]->funcType == asFUNC_SYSTEM )
        {
            ...
            else PrepareSystemFunction(scriptFunctions[n], scriptFunctions[n]->sysFuncIntf, this);
        }
    }
...
int asCScriptEngine::ConfigError(int err, const char *funcName, const char *arg1, const char *arg2)
{
    ...
    if( funcName )
    {
        ...
        if( arg1 )
        {
            if( arg2 ) str.Format(TXT_FAILED_IN_FUNC_s_WITH_s_AND_s_s_d, funcName, arg1, arg2, errorNames[-err], err);
...
{
    if (factory == 0) return ConfigError(asINVALID_ARG, "RegisterStringFactory", datatype, 0);

    // Parse the data type
    asCBuilder bld(this, 0);
    asCDataType dt;
    int r = bld.ParseDataType(datatype, &dt, defaultNamespace, true);
    if (r < 0) return ConfigError(asINVALID_TYPE, "RegisterStringFactory", datatype, 0);

    // Validate the type. It must not be reference or handle
    if (dt.IsReference() || dt.IsObjectHandle()) return ConfigError(asINVALID_TYPE, "RegisterStringFactory", datatype, 0);
...
{
    if( stringFactory == 0 ) return asNO_FUNCTION;
...
{
    if( name == 0 ) name = ""; // Accept null as well as zero-length string
    ...
    if( create )
    {
        ...
        if( retModule == 0 ) return 0; // Out of memory
...
void asCScriptEngine::RemoveTemplateInstanceType(asCObjectType *t)
{
    ...
    if( t->externalRefCount.get() ) return;
...
asCDataType asCScriptEngine::DetermineTypeForTemplate(const asCDataType &orig, asCObjectType *tmpl, asCObjectType *ot)
{
    ...
    else if( orig.GetTypeInfo() == tmpl )
    {
        ...
        else dt = asCDataType::CreateType(ot, false);
    ...
    else if( orig.GetTypeInfo() && (orig.GetTypeInfo()->flags & asOBJ_TEMPLATE) )
    {
        ...
        else dt = asCDataType::CreateType(ntype, false);
...
bool asCScriptEngine::GenerateNewTemplateFunction( asCObjectType *templateType, asCObjectType *ot, asCScriptFunction *func, asCScriptFunction **newFunc)
{
    ...
    if( func2 == 0 ) return false; // Out of memory
    ...
    for (asUINT n = 0; n < func->defaultArgs.GetLength(); n++)
        ...
        else func2->defaultArgs.PushLast(0);
    ...
    else PrepareSystemFunction(func2, func2->sysFuncIntf, this);
...
asCFuncdefType *asCScriptEngine::GenerateNewTemplateFuncdef(asCObjectType *templateType, asCObjectType *ot, asCFuncdefType *func)
{
    ...
    if (func2 == 0) return 0; // Out of memory
...
void asCScriptEngine::CallObjectMethod(void *obj, asSSystemFunctionInterface *i, asCScriptFunction *s) const
{
    ...
    else if( i->callConv == ICC_THISCALL || i->callConv == ICC_VIRTUAL_THISCALL )
    {
        ...
        if(i->isCompositeIndirect) obj = *((void**)obj);
...
int asCScriptEngine::GetTypeIdFromDataType(const asCDataType &dtIn) const
{
    ...
    if( dtIn.GetTypeInfo() && !(dtIn.GetTypeInfo()->flags & asOBJ_ASHANDLE) )
    {
        ...
        if( dtIn.IsObjectHandle() )  typeId |= asTYPEID_OBJHANDLE;
        if( dtIn.IsHandleToConst() ) typeId |= asTYPEID_HANDLETOCONST;
    }
    return typeId;
}
...
asCDataType asCScriptEngine::GetDataTypeFromTypeId(int typeId) const
{
    ...
    if( mapTypeIdToTypeInfo.MoveTo(&cursor, baseId) ) ot = mapTypeIdToTypeInfo.GetValue(cursor);

    if( ot )
    {
        asCDataType dt = asCDataType::CreateType(ot, false);
        if( typeId & asTYPEID_OBJHANDLE )     dt.MakeHandle(true, true);
        if( typeId & asTYPEID_HANDLETOCONST ) dt.MakeHandleToConst(true);
...
//            WriteMessage("", 0, 0, asMSGTYPE_ERROR, str.AddressOf());
        ...
        if (funcIndex)
        {
            if (objType->flags & asOBJ_TEMPLATE) CallScriptObjectMethod( ptr, funcIndex ); // Templates of value types create script functions as the constructors
            else                                 CallObjectMethod(ptr, funcIndex);
        }
    }
    return ptr;
}
...
int asCScriptEngine::CallScriptObjectMethod( void *obj, int funcId )
{
    ...
    if (ctx == 0)
    {
        ...
        if (ctx == 0) return asERROR; // TODO: How to best report this failure?
    }
    ...
    if (r != asEXECUTION_FINISHED)
    {
        if (isNested)
        {
            ...
            if (r == asEXECUTION_EXCEPTION)    ctx->SetException(TXT_EXCEPTION_IN_NESTED_CALL); // TODO: How to improve this exception
...
void asCScriptEngine::ConstructScriptObjectCopy( void *mem, void *obj, asCObjectType *type )
{
    ...
    else
    {
        ...
        if( funcIndex ) CallObjectMethod(mem, funcIndex);
...
int asCScriptEngine::AssignScriptObject(void *dstObj, void *srcObj, const asITypeInfo *type)
{
    ...
    if (ep.disallowValueAssignForRefType && (objType->flags & asOBJ_REF) && !(objType->flags & asOBJ_SCOPED))
    {
        ...
        if (ctx) ctx->SetException("Cannot do value assignment");
        return asNOT_SUPPORTED;
    }
...
asCConfigGroup *asCScriptEngine::FindConfigGroupForTypeInfo(const asCTypeInfo *objType) const
{
    for( asUINT n = 0; n < configGroups.GetLength(); n++ )
    {
        ...
        {
            if( configGroups[n]->types[m] == objType ) return configGroups[n];
        }
    }
    return 0;
}
...
int asCScriptEngine::RegisterFuncdef( const char *decl )
{
    ...
    if( func == 0 ) return ConfigError(asOUT_OF_MEMORY, "RegisterFuncdef", decl, 0);
...
int asCScriptEngine::RegisterTypedef( const char *type, const char *decl )
{
    ...
    default: return ConfigError(asINVALID_TYPE, "RegisterTypedef", type, decl);
    }
...
{
    //    Check the name
    if( NULL == name ) return ConfigError(asINVALID_NAME, "RegisterEnum", name, 0);
    ...
    r = bld.CheckNameConflict(name, 0, 0, defaultNamespace, true, false);
    if( r < 0 ) return ConfigError(asNAME_TAKEN, "RegisterEnum", name, 0);

    asCEnumType *st = asNEW(asCEnumType)(this);
    if( st == 0 ) return ConfigError(asOUT_OF_MEMORY, "RegisterEnum", name, 0);
...
int asCScriptEngine::RegisterEnumValue( const char *typeName, const char *valueName, int value )
{
    ...
    if( r < 0 ) return ConfigError(r, "RegisterEnumValue", typeName, valueName);

    // Store the enum value
    asCEnumType *ot = CastToEnumType(dt.GetTypeInfo());
    if( ot == 0 ) return ConfigError(asINVALID_TYPE, "RegisterEnumValue", typeName, valueName);
...
{
    if( in_name == 0 ) return asINVALID_ARG;
...
{
    asCDataType dt = GetDataTypeFromTypeId(typeId);
    if (!dt.IsValid()) return 0; // Is the type id valid?
    return dt.GetTypeInfo();
}
    ...
    while( node )
    {
        if( node->type == asLPT_REPEAT || node->type == asLPT_REPEAT_SAME )
        {
            ...
            if( count == 0 )
            {
                ...
                if( node->type == asLPT_START )
                {
                    ...
                    do{
                        node = node->next;
                        if     ( node->type == asLPT_START ) subCount++;
                        else if( node->type == asLPT_END   ) subCount--;
                    } while( subCount > 0 );
                    return;
                }
            }
            ...
            while( count-- )
            {
                ...
                if( ti && (ti->flags & asOBJ_ENUM) == 0 )
                {
                    ...
                    if( ti->flags & asOBJ_VALUE )
                    {
                        ...
                        // Call the release behaviour
                        void *ptr = *(void**)buffer;
                        if( ptr ) ReleaseScriptObject(ptr, ti);
        ...
        else if( node->type == asLPT_START )
        {
            ...
            while( count-- )
            {
                ...
                if( count == 0 ) node = subList;
            }
        }
        else if( node->type == asLPT_END ) return;
        else                               asASSERT( false );
