...
void asPopActiveContext(asCThreadLocalData *tld, asIScriptContext *ctx)
{
    ...
    if (tld) tld->activeContexts.PopLast();
}
...
bool asCContext::IsNested(asUINT *nestCount) const
{
    ...
    if( nestCount && *nestCount > 0 ) return true;
...
void *asCContext::GetReturnAddress()
{
    ...
    if( dt->IsReference() ) return *(void**)&m_regs.valueRegister;
...
void *asCContext::GetReturnObject()
{
    ...
    if( dt->IsReference() ) return *(void**)(asPWORD)m_regs.valueRegister;
    else{
		if( m_initialFunction->DoesReturnOnStack() )
		{
			// The address of the return value was passed as the first argument, after the object pointer
...
int asCContext::SetArgObject( asUINT arg, void *obj )
{
	...
	if( !dt->IsReference() )
	{
		if( dt->IsObjectHandle() )
		{
            ...
            if (obj && dt->IsFuncdef()) ((asIScriptFunction*)obj)->AddRef();
			else
			{
        ...
        else obj = m_engine->CreateScriptObjectCopy( obj, dt->GetTypeInfo() );
	}
...
int asCContext::Abort()
{
    ...
    if( m_status == asEXECUTION_SUSPENDED ) m_status = asEXECUTION_ABORTED;
...
bool asCContext::ReserveStackSpace( asUINT size )
{
    if( m_stackBlocks.GetLength() == 0 ) // Make sure the first stack block is allocated
	{
        ...
        if( stack == 0 ) return false; // Out of memory
...
void asCContext::CleanStack(bool catchException)
{
    ...
    if (caught) m_status = asEXECUTION_ACTIVE;
...
void asCContext::CleanArgsOnStack()
{
	...
	else if( bc == asBC_CallPtr )
	{
		...
		if( func == 0 )
		{
			// Look in parameters
			int paramPos = 0;
            if( m_currentFunction->objectType          ) paramPos -= AS_PTR_SIZE;
            if( m_currentFunction->DoesReturnOnStack() ) paramPos -= AS_PTR_SIZE;
	...
	// Clean parameters
	int offset = 0;
    if( func->objectType          ) offset += AS_PTR_SIZE;
    if( func->DoesReturnOnStack() ) offset += AS_PTR_SIZE;
...
bool asCContext::CleanStackFrame( bool catchException )
{
    ...
    else m_isStackMemoryNotAllocated = false;
	...
	int offset = 0;
    if( m_currentFunction->objectType          ) offset += AS_PTR_SIZE;
    if( m_currentFunction->DoesReturnOnStack() ) offset += AS_PTR_SIZE;
...
void asCContext::CallLineCallback()
{
    ...
    else m_engine->CallObjectMethod(m_lineCallbackObj, this, &m_lineCallbackFunc, 0);
}
	...
	if( cleanCount )
	{
		...
		for( asUINT n = 0; n < cleanCount; n++, clean++ )
		{
			...
			if( clean->op == 0 )
			{
                ...
                if( clean->op == 2 ) m_engine->CallObjectMethod(*addr, clean->ot->beh.destruct);
...
void *asCContext::GetAddressOfVar(asUINT varIndex, asUINT stackLevel)
{
	...
	if( (func->scriptData->variables[varIndex]->type.IsObject() && !func->scriptData->variables[varIndex]->type.IsObjectHandle()) || (pos <= 0) )
	{
		...
		if( !onHeap && pos <= 0 )
		{
			...
			int stackPos = 0;
            if( func->objectType          ) stackPos -= AS_PTR_SIZE;
            if( func->DoesReturnOnStack() ) stackPos -= AS_PTR_SIZE;

			for( asUINT n = 0; n < func->parameterTypes.GetLength(); n++ )
			{
                if( stackPos == pos ) // The right argument was found. Is this a reference parameter?
                {
                    if( func->inOutFlags[n] != asTM_NONE ) onHeap = true;
					break;
				}
				stackPos -= func->parameterTypes[n].GetSizeOnStackDWords();
			}
		}
        if( onHeap ) return *(void**)(sf - func->scriptData->variables[varIndex]->stackOffset);
	}
...
int asCContext::GetThisTypeId(asUINT stackLevel)
{
    ...
    if( func->GetObjectType() == 0 ) return 0; // not in a method
...
void *asCContext::GetThisPointer(asUINT stackLevel)
{
    ...
    if( func == 0             ) return 0;
    if( func->objectType == 0 ) return 0; // not in a method

	void *thisPointer = (void*)*(asPWORD*)(sf);
    if( thisPointer == 0 ) return 0;
...
int as_powi(int base, int exponent, bool& isOverflow)
{
	if( exponent < 0 )
    {
        if( base == 0 ) isOverflow = true; // Divide by zero
        else            isOverflow = false; // Result is less than 1, so it truncates to 0
	...
	else if( exponent >= 31 )
	{
		switch( base )
		{
        case -1: isOverflow = false; return exponent & 1 ? -1 : 1;
        case 0:  isOverflow = false; break;
        case 1:  isOverflow = false; return 1;
        default: isOverflow = true;  break;
		}
		return 0;
	}
	else
